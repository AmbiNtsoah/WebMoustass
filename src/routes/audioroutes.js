...
const upload = multer({
 ...
 router.get('/messages', async (req, res) => {
   try {
     const [rows] = await connexion.query('SELECT id, filename, created_at FROM messages ORDER BY id ASC');
     res.json(rows);
   } catch (err) {
 ...
 router.delete('/delete-message/:id', async (req, res) => {
   ...
   try {
     const [rows] = await connexion.query('SELECT filename FROM messages WHERE id = ?', [messageId]);
     ...
     await connexion.query('DELETE FROM messages WHERE id = ?', [messageId]);
 ...
 router.get('/get-audio/:id', async (req, res) => {
   try {
     ...
     const [rows] = await connexion.query('SELECT * FROM messages WHERE id = ?', [messageId]);
